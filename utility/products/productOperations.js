...
async function createFinishingMatrixRowQuantityPrices(finishingMatrixRowId, quantityDetails) {
  ...
  let query =
    "insert into finishingMatrixRowQuantityPrices (finishingMatrixRowFk, quantityFk, price, deleteFl, versionNo) values ";
  quantityDetails.forEach((q) => {
...
async function getQuantityPriceTable(options, finishingOptions, productId) {
  let query =
    "select q.quantity, q.id as quantityId, pmrqpr.id as priceMatrixRowQuantityRowId, pmrqpr.price, pmrqpr.price/q.quantity as pricePer from priceMatrixRowQuantityPrices pmrqpr " +
...
async function validateProductInformationDetails(productDetails) {
  const errors = {};
  const { name, productTypeFk, image1Path, description, subDescriptionTitle, subDescription, descriptionPoint1 } =
    productDetails;
...
async function getSaleForProductId(productFk, isActive) {
  ...
  let query = 'select *, DATE_FORMAT(fromDt, "%Y-%m-%d") as fromDt, DATE_FORMAT(toDt, "%Y-%m-%d") as toDt from sales where productFk = :productId and deleteFl = false ';
  ...
  const sales = await models.sequelize.query(query, {
    replacements: {productId: productFk, now: new Date()}, type: models.sequelize.QueryTypes.SELECT
  });
...
async function createSaleForProductId(productFk, name, fromDt, toDt, description, percentage) {
...
async function updateSaleForProductId(productFk, name, fromDt, toDt, description, percentage) {
...
async function deleteSaleByProductId(productFk, id) {
...
module.exports = {
  ...
  getSaleForProductId,
  createSaleForProductId,
  updateSaleForProductId,
  deleteSaleByProductId,
